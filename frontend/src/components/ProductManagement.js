...
const ProductManagement = ({ products, onUpdate, apiUrl }) => {
  const [formData, setFormData] = useState({
    name: '', description: '', category: '', price: '', quantity: '', image: null
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProduct && editingProduct.imageUrl) formDataToSend.append('currentImage', editingProduct.imageUrl);
  ...
  const handleEdit = (product) => {
    ...
    setImagePreview(product.imageUrl ? `${apiUrl}${product.imageUrl}` : null);
  };
          ...
          <div className="products-grid">
            {products.map(product => (
              <div key={product.id} className="product-card">
                {product.imageUrl && <div className="product-image"><img src={`${apiUrl}${product.imageUrl}`} alt={product.name} /></div>}
                <div className="product-info">
                  <h3>{product.name}</h3>
                  <p className="product-category">{product.category}</p>
                  <p className="product-price">M{product.price.toFixed(2)}</p>
                  <p className={`product-quantity ${product.quantity < 10 ? 'low-stock' : ''}`}>Stock: {product.quantity}</p>
